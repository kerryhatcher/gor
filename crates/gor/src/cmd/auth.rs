...
use anyhow::Context;
use gor_core::auth::{device, token};
use gor_core::client::Client;
use gor_core::host::Host;
...
fn logout(hostname: &str) -> anyhow::Result<()> {
    ...
    gor_core::keyring_store::delete_token(hostname)
...
fn token(hostname: &str, refresh: bool, scopes: &[String], secure: bool) -> anyhow::Result<()> {
    if refresh {
        ...
        // Store the new token.
        gor_core::keyring_store::set_token(hostname, &access_token)
    ...
    let token = gor_core::keyring_store::get_token(hostname)
